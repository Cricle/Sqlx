...
public class RepositoryGenerator : IIncrementalGenerator
{
        ...
        Compilation compilation)
    {
        ...
        var entityFullName = entityType?.ToDisplayString() ?? "object";
        var entityName = entityType?.Name ?? "Entity";
        var keyType = GetKeyType(serviceType);
        var keyTypeName = keyType?.ToDisplayString() ?? "int";
        var entityNs = entityType?.ContainingNamespace.IsGlobalNamespace == false ? entityType.ContainingNamespace.ToDisplayString() : null;
        ...
        foreach (var method in methods)
        {
            ...
            if (actualReturnType is INamedTypeSymbol namedType && 
        ...
        // Add all entity namespaces
        foreach (var entityNamespace in entityNamespaces.OrderBy(n => n))
        {
        ...
        // Check if user has defined constructor with SqlDialect parameter
        var hasUserDialectConstructor = repoType.HasDialectConstructor(out var userCtor);
        var primaryCtorDialectParameterName = repoType.GetPrimaryConstructorDialectParameterName();

        // Generate runtime dialect field and property
        RuntimeDialectCodeGen.GenerateDialectField(sb, primaryCtorDialectParameterName);

        // Find or generate DbConnection field
        var connectionInfo = FindOrGenerateDbConnection(repoType, sb, hasUserDialectConstructor, repoName);
        ...
        // Generate runtime PlaceholderContext (lazy-initialized)
        RuntimeDialectCodeGen.GenerateRuntimeContext(sb, entityName, tableName, entityType != null);

        // Generate table name constant
        sb.AppendLine($"private const string _tableName = \"{tableName}\";");
        sb.AppendLine();

        // Generate runtime support for {{var}} placeholders when needed
        GeneratePlaceholderContext(sb, repoType, entityName, entityType != null, methods, sqlTemplateAttr);
    ...
    /// Also generates constructor with SqlDialect parameter if needed.
    /// Priority: field > property > primary constructor parameter > generate default property
    /// </summary>
    private static ConnectionInfo FindOrGenerateDbConnection(
        INamedTypeSymbol repoType, 
        ...
        string repoName)
    {
        ...
        // No class-level connection found, generate a default property and constructor
        sb.AppendLine("/// <summary>Database connection (auto-generated).</summary>");
        sb.AppendLine("private global::System.Data.Common.DbConnection _connection = null!;");
        sb.AppendLine();
        sb.AppendLine("/// <summary>Gets or sets the database connection.</summary>");
        sb.AppendLine("global::System.Data.Common.DbConnection? ISqlxRepository.Connection");
        sb.AppendLine("{");
        sb.PushIndent();
        sb.AppendLine("get => _connection;");
        sb.AppendLine("set => _connection = value!;");
        ...
        RuntimeDialectCodeGen.GenerateConstructorIfNeeded(sb, repoName, hasUserDialectConstructor, "_connection");
    ...
    private static void GeneratePlaceholderContext(
        ...
        INamedTypeSymbol? sqlTemplateAttr)
    {
        var requiresDynamicContext = methods.Any(m =>
        {
            var template = GetSqlTemplate(m, sqlTemplateAttr);
            return template != null && UsesVarPlaceholder(template);
        });

        if (!requiresDynamicContext)
        {
            return;
        }

        sb.AppendLine("// Dynamic context - cached instance for {{var}} placeholder support");
        ...
        sb.AppendLine("tableName: _tableName,");
        if (hasEntity)
        {
    ...
    /// <summary>
    /// Generates parameter name members for all scalar parameters.
    ...
    private static Dictionary<string, string> GenerateParameterNameFields(IndentedStringBuilder sb, List<IMethodSymbol> methods, string entityName)
    {
        ...
        sb.AppendLine("// Instance parameter names (dialect-aware)");
        var result = new Dictionary<string, string>();
        foreach (var name in paramNames.OrderBy(n => n))
        {
            var fieldName = $"_param_{name}";
            result[name] = fieldName;
            sb.AppendLine($"private string {fieldName} => ParamPrefix + \"{name}\";");
        }
    ...
    {
        sb.AppendLine("// Cached SqlTemplate fields (runtime dialect aware)");
        ...
        foreach (var method in methods)
        {
            ...
            RuntimeDialectCodeGen.GenerateRuntimeTemplateField(sb, fieldName, template, UsesVarPlaceholder(template));
        }
    }
        ...
        Dictionary<string, string> paramNameFields)
    {
        ...
        var templateReferenceName = "__template";

        if (returnType == "Sqlx.SqlTemplate" || returnType == "global::Sqlx.SqlTemplate" || returnType.EndsWith(".SqlTemplate"))
        {
            GenerateSqlTemplateReturnMethod(sb, method, fieldName, template!, usesVar, templateReferenceName);
            return;
        }
        ...
        if (template != null)
        {
            RuntimeDialectCodeGen.GenerateGetTemplate(sb, fieldName, template, usesVar);
        ...
        if (usesVar)
        {
            sb.AppendLine($"cmd.CommandText = {templateReferenceName}.Render(null);");
        }
        ...
        {
            sb.AppendLine($"cmd.CommandText = {templateReferenceName}.Sql;");
        }
        ...
        sb.AppendLine($"OnExecuting(\"{methodName}\", cmd, {templateReferenceName});");
        sb.AppendLine("#endif");
        sb.AppendLine();
    ...
    private static void GenerateSimpleParameterBinding(IndentedStringBuilder sb, IMethodSymbol method, Dictionary<string, string> paramNameFields)
    {
        ...
        foreach (var param in method.Parameters)
        {
            ...
            var paramFieldName = paramNameFields.TryGetValue(param.Name, out var fn) ? fn : $"ParamPrefix + \"{param.Name}\"";
    ...
    private static void GenerateDynamicParamsDeclaration(IndentedStringBuilder sb, IMethodSymbol method)
    {
        ...
        if (expressionParams.Count == 1)
        {
            ...
            {
                sb.AppendLine($"var {param.Name}Sql = global::Sqlx.SetExpressionExtensions.ToSetClause({param.Name}, Dialect);");
                sb.AppendLine($"var {param.Name}Params = global::Sqlx.SetExpressionExtensions.GetSetParameters({param.Name});");
            }
            else
            {
                sb.AppendLine($"var {param.Name}Sql = global::Sqlx.ExpressionExtensions.ToWhereClause({param.Name}, Dialect);");
            }
        }
        ...
        else
        {
            ...
            foreach (var param in expressionParams)
            {
                ...
                if (IsSetExpression(paramType))
                {
                    sb.AppendLine($"[\"{param.Name}\"] = global::Sqlx.SetExpressionExtensions.ToSetClause({param.Name}, Dialect),");
                }
                else
                {
                    sb.AppendLine($"[\"{param.Name}\"] = global::Sqlx.ExpressionExtensions.ToWhereClause({param.Name}, Dialect),");
                }
            }
    ...
    private static void GenerateActivityStart(IndentedStringBuilder sb, string methodName, IMethodSymbol method, bool hasDynamicParams)
    {
        ...
        sb.AppendLine("activity.SetTag(\"db.system\", Dialect.DatabaseType);");
    ...
    private static void GenerateParameterBinding(IndentedStringBuilder sb, IMethodSymbol method, string entityName, INamedTypeSymbol? expressionToSqlAttr, Dictionary<string, string> paramNameFields)
    {
        ...
        if (setExpressionParams.Any())
        {
            ...
            foreach (var param in setExpressionParams)
            {
                ...
                sb.PushIndent();
                sb.AppendLine("var p = cmd.CreateParameter();");
                sb.AppendLine("p.ParameterName = ParamPrefix + kvp.Key;");
    ...
    private static void GenerateExecuteAndReturn(IndentedStringBuilder sb, IMethodSymbol method, string repoFullName, string entityFullName, string entityName, string keyTypeName, bool isReturnInsertedId, string methodName, string fieldName, bool useStaticOrdinals, string ordinalsFieldName, bool isAsync, bool useSimpleScalarExecution, Dictionary<string, string> paramNameFields)
    {
        ...
        // Determine the actual return type name to use
        // Priority: 1) actualReturnType if entityType is null (no ICrudRepository interface)
        //           2) actualReturnType if it's different from entityType (custom return type)
        //           3) entityType (standard case)
        string actualReturnTypeName;
        string actualReturnTypeFullName;
        bool isCustomReturnType;
        
        if (entityType == null && actualReturnType != null && actualReturnType.TypeKind == TypeKind.Class)
        {
            // No entity type from interface, use actual return type from method
            actualReturnTypeName = actualReturnType.Name;
            actualReturnTypeFullName = actualReturnType.ToDisplayString();
            isCustomReturnType = true; // Treat as custom since we don't have a base entity type
        }
        else if (actualReturnType != null && entityType != null && 
                 !SymbolEqualityComparer.Default.Equals(actualReturnType, entityType) &&
                 actualReturnType.TypeKind == TypeKind.Class)
        {
        ...
        {
            // Standard case: use entity type
            actualReturnTypeName = entityName;
            actualReturnTypeFullName = entityFullName;
    ...
    private static void GenerateSqlTemplateReturnMethod(
        IndentedStringBuilder sb,
        IMethodSymbol method,
        string fieldName,
        string template,
        ...
        string templateReferenceName)
    {
        ...
        RuntimeDialectCodeGen.GenerateGetTemplate(sb, fieldName, template, usesVar);
        sb.AppendLine(GetTemplateLocalDeclaration(templateReferenceName, fieldName));
        sb.AppendLine($"return {templateReferenceName};");
        sb.PopIndent();
        sb.AppendLine("}");
    }
    ...
    private static void GenerateIQueryableReturnMethod(IndentedStringBuilder sb, IMethodSymbol method, string entityFullName, string connectionExpression)
    {
        ...
        sb.AppendLine("{");
        sb.PushIndent();
        sb.AppendLine($"return global::Sqlx.SqlQuery<{entityFullName}>.For(Dialect).WithConnection({connectionExpression});");
        sb.PopIndent();
        sb.AppendLine("}");
    }
    ...
    private static void GenerateTupleReturn(IndentedStringBuilder sb, IMethodSymbol method, string repoFullName, string entityName, string ctName, string methodName, string fieldName, Dictionary<string, string> paramNameFields)
    {
        var tupleElements = GetTupleElements(method.ReturnType);
        if (tupleElements.Count == 0) return;

        // Parse ResultSetMapping attributes or use default mappings
        var mappings = ParseResultSetMappings(method);
        if (mappings.Count == 0)
        {
            mappings = GetDefaultResultSetMappings(tupleElements);
        }

        // Check if this is a scalar result set method (all elements are scalar types)
        var allScalar = tupleElements.All(e => IsScalarType(e.Type));

        if (allScalar)
        {
            // Generate scalar result set reading code
            GenerateScalarTupleReturn(sb, method, tupleElements, mappings, ctName, methodName, fieldName, repoFullName, paramNameFields);
        }
        else
        {
            // Generate entity result set reading code (existing behavior)
            GenerateEntityTupleReturn(sb, method, tupleElements, ctName, methodName, fieldName, repoFullName, entityName);
        }
    }

    /// <summary>
    /// Generates code for tuple return with scalar values (int, long, string, etc.).
    /// Supports ResultSetMapping attribute to map result sets to tuple elements.
    /// </summary>
    private static void GenerateScalarTupleReturn(
        IndentedStringBuilder sb,
        IMethodSymbol method,
        List<(ITypeSymbol Type, string? Name)> tupleElements,
        ...
        Dictionary<string, string> paramNameFields)
    {
        ...
        // Declare variables for all tuple elements
        foreach (var (elementType, elementName) in tupleElements)
        {
            var varName = elementName ?? "Item1";
            var typeName = elementType.ToDisplayString();
            sb.AppendLine($"{typeName} {varName};");
        }
        sb.AppendLine();

        // Always use ExecuteReader for multiple result sets
        // We can get affected rows from reader.RecordsAffected
        if (isAsync)
        {
            sb.AppendLine($"using var reader = await cmd.ExecuteReaderAsync(System.Data.CommandBehavior.Default, {ctName}).ConfigureAwait(false);");
        }
        ...
        var sortedIndices = mappings.Keys.OrderBy(k => k).ToList();
        
        // Track which result set we're reading (0-based, excludes affected rows)
        int resultSetIndex = 0;
        
        for (int i = 0; i < sortedIndices.Count; i++)
        {
            var index = sortedIndices[i];
            var elementName = mappings[index];
            var element = tupleElements.FirstOrDefault(e => 
                (e.Name ?? "").Equals(elementName, StringComparison.OrdinalIgnoreCase));

            if (element.Type == null) continue;

            // Check if this is the affected rows (index 0 and name suggests it's affected rows)
            var isAffectedRows = index == 0 && 
                (element.Type.ToDisplayString() == "int" || element.Type.ToDisplayString() == "System.Int32") &&
                (elementName.Equals("rowsAffected", StringComparison.OrdinalIgnoreCase) || 
                 elementName.Equals("affectedRows", StringComparison.OrdinalIgnoreCase) ||
                 elementName.Equals("rows", StringComparison.OrdinalIgnoreCase));

            if (isAffectedRows)
            {
                // Get affected rows from reader.RecordsAffected
                sb.AppendLine($"// Get affected rows from reader");
                sb.AppendLine($"{elementName} = reader.RecordsAffected;");
                sb.AppendLine();
            }
            else
            {
                // Read scalar value from result set
                GenerateScalarResultSetRead(sb, element.Type, elementName, index, resultSetIndex, isAsync, ctName);
                resultSetIndex++;
            }
        }
        ...
        sb.AppendLine("#if !SQLX_DISABLE_INTERCEPTOR");
        sb.AppendLine("var elapsed = Stopwatch.GetTimestamp() - startTime;");
        sb.AppendLine($"OnExecuted(\"{methodName}\", cmd, {fieldName}, ({string.Join(", ", varNames)}), elapsed);");
        sb.AppendLine("#endif");
        sb.AppendLine($"global::Sqlx.Diagnostics.SqlTemplateMetrics.RecordExecution(\"{repoFullName}\", \"{methodName}\", {fieldName}.TemplateSql, Stopwatch.GetTimestamp() - startTime);");
        sb.AppendLine();
        sb.AppendLine($"return ({string.Join(", ", varNames)});");
    }

    /// <summary>
    /// Generates code to read a single scalar value from a result set.
    /// </summary>
    private static void GenerateScalarResultSetRead(
        IndentedStringBuilder sb,
        ...
        string ctName)
    {
        ...
        sb.AppendLine($"// Read result set {index}: {elementName}");
        ...
        if (resultSetIndex > 0)
        {
            ...
            sb.AppendLine($"    throw new InvalidOperationException(\"No data returned for result set {index}\");");
        }
        ...
        // Read value and convert to target type
        if (isNullable)
        {
            sb.AppendLine($"{elementName} = reader.IsDBNull(0) ? ({typeName})null : ({baseTypeName})Convert.ChangeType(reader.GetValue(0), typeof({baseTypeName}));");
        }
        else
        {
            sb.AppendLine($"{elementName} = ({typeName})Convert.ChangeType(reader.GetValue(0), typeof({typeName}));");
        }
        sb.AppendLine();
    }

    /// <summary>
    /// Generates code for tuple return with entity types (existing behavior).
    /// </summary>
    private static void GenerateEntityTupleReturn(
        IndentedStringBuilder sb,
        IMethodSymbol method,
        List<(ITypeSymbol Type, string? Name)> tupleElements,
        string ctName,
        ...
        string entityName)
    {
        ...
        for (int i = 0; i < tupleElements.Count; i++)
        {
            ...
            if (IsListType(elementType))
            {
                ...
                {
                    sb.AppendLine($"var {varName} = {listElementName}ResultReader.Default.ToList(reader);");
                }
            }
            else
            {
                ...
                {
                    sb.AppendLine($"var {varName} = {elementEntityName}ResultReader.Default.FirstOrDefault(reader);");
                }
            }
    ...
    /// <summary>
    /// Parses ResultSetMapping attributes from a method.
    /// Returns a dictionary mapping index to tuple element name.
    /// </summary>
    private static Dictionary<int, string> ParseResultSetMappings(IMethodSymbol method)
    {
    ...
    /// <summary>
    /// Gets the default result set mappings when no ResultSetMapping attributes are present.
    /// First element maps to affected rows (index 0), remaining elements map to SELECT results (index 1+).
    /// </summary>
    private static Dictionary<int, string> GetDefaultResultSetMappings(List<(ITypeSymbol Type, string? Name)> tupleElements)
    {
        var mappings = new Dictionary<int, string>();
        
        for (int i = 0; i < tupleElements.Count; i++)
        {
            var elementName = tupleElements[i].Name ?? $"Item{i + 1}";
            mappings[i] = elementName;
        }
    ...
    /// <summary>
    /// Checks if a type is a scalar type (int, long, string, etc.) rather than an entity type.
    /// </summary>
    private static bool IsScalarType(ITypeSymbol type)
    {
        var typeName = type.ToDisplayString();
        return typeName == "int" || typeName == "System.Int32" ||
