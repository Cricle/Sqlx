// -----------------------------------------------------------------------
// <copyright file="SqlServerFeaturePropertyTests.cs" company="Cricle">
...
/// <summary>
/// Property-based tests for SQL Server-specific features.
/// **Feature: enhanced-predefined-interfaces-e2e-testing**
/// **Validates: Requirements 4.4, 4.5, 4.8, 4.9**
/// </summary>
public class SqlServerFeaturePropertyTests : IDisposable
{
    private readonly IDbConnection? _connection;
    private readonly E2EUserCommandRepository_SqlServer? _commandRepo;
    private readonly E2EUserQueryRepository_SqlServer? _queryRepo;
    private readonly E2EUserBatchRepository_SqlServer? _batchRepo;
    private readonly bool _isAvailable;
    private const string TestClassName = nameof(SqlServerFeaturePropertyTests);

    public SqlServerFeaturePropertyTests()
    {
        _connection = DatabaseConnectionHelper.GetSqlServerConnection(TestClassName);
        ...
        if (_isAvailable)
        {
            ...
            cmd.CommandText = @"
                IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'e2e_users')
                BEGIN
            ...
            _commandRepo = new E2EUserCommandRepository_SqlServer(_connection);
            _queryRepo = new E2EUserQueryRepository_SqlServer(_connection);
            _batchRepo = new E2EUserBatchRepository_SqlServer(_connection);
        }
    }

    /// <summary>
    /// **Property 18: Upsert Idempotence (SQL Server-specific)**
    /// *For any* entity, upserting it multiple times should produce the same result
    /// as upserting it once (idempotent operation using MERGE).
    ...
    public Property Upsert_ShouldBeIdempotent(E2EUserData userData)
    {
        ...
        // Act - Upsert multiple times using MERGE
    ...
    /// <summary>
    /// **Property 30: Boolean Preservation (SQL Server-specific)**
    /// *For any* entity with boolean fields, inserting and retrieving should preserve
    /// the exact boolean values (SQL Server stores as BIT 0/1).
    ...
    public Property Boolean_ShouldBePreservedInRoundTrip(E2EUserData userData)
    {
        ...
        // Assert - Boolean values should be preserved exactly (BIT 0/1 in SQL Server)
    ...
    /// <summary>
    /// **Property 30b: Boolean Preservation in Batch Operations (SQL Server-specific)**
    /// *For any* list of entities with boolean fields, batch inserting and retrieving
    /// should preserve all boolean values correctly (SQL Server BIT 0/1).
    /// **Validates: Requirements 4.8**
    /// </summary>
    [Property(MaxTest = 10, Arbitrary = new[] { typeof(E2EUserArbitraries) })]
    ...
    /// <summary>
    /// **Property 32: Random Query Validity (SQL Server-specific)**
    /// *For any* non-empty table, GetRandomAsync should always return valid entities
    /// that exist in the table (using NEWID() function).
    ...
    public Property GetRandom_ShouldReturnValidEntities(PositiveInt count, NonEmptyArray<E2EUserData> userDataArray)
    {
        ...
        // Act - Get random users using NEWID()
    ...
    /// <summary>
    /// **Property 32b: Random Query Uniqueness (SQL Server-specific)**
    /// *For any* table with sufficient records, GetRandomAsync should tend to return
    /// different results on multiple calls (statistical property using NEWID()).
    /// **Validates: Requirements 4.9**
    /// </summary>
    ...
    public Property GetRandom_ShouldTendToReturnDifferentResults(NonEmptyArray<E2EUserData> userDataArray)
    {
        ...
        // Act - Get random users multiple times using NEWID()
    ...
    /// <summary>
    /// **Property 18b: Batch Upsert Correctness (SQL Server-specific)**
    /// *For any* list of entities (mix of existing and new), batch upsert should
    /// correctly update existing records and insert new ones using MERGE.
    /// **Validates: Requirements 4.5**
    /// </summary>
    [Property(MaxTest = 10, Arbitrary = new[] { typeof(E2EUserArbitraries) })]
    public Property BatchUpsert_ShouldCorrectlyHandleMixedOperations(NonEmptyArray<E2EUserData> existingData, NonEmptyArray<E2EUserData> newData)
    {
        ...
        // Add modified existing users (deduplicate by ID)
        ...
        // Deduplicate by ID (keep last occurrence) - avoid duplicate key issues
    ...
    private async System.Threading.Tasks.Task ClearTableAsync()
    {
        ...
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "TRUNCATE TABLE e2e_users";
