...
|------|------|------------|---------|
| 单条查询 | **9.06 μs** | 10.14 μs (+12%) | 63.87 μs (+605%) |
| 内存分配 | **1.79 KB** | 2.96 KB (+65%) | 11.48 KB (+541%) |

**AOT 兼容性：** ✅ 完全支持 Native AOT，通过 817 个单元测试
...
|--------|------|-------|--------|-------|-----------|-------------|
| **Sqlx** | **9.06 μs** | 4.47 μs | 0.24 μs | **1.00** | **1.79 KB** | **1.00** |
| Dapper.AOT | 10.14 μs | 2.96 μs | 0.16 μs | 1.12 | 2.96 KB | 1.66 |
| FreeSql | 63.87 μs | 30.15 μs | 1.65 μs | 7.06 | 11.48 KB | 6.42 |

**结论：** Sqlx 比 Dapper.AOT 快 12%，比 FreeSql 快 7.1 倍。

**性能提升：** 相比之前版本，Sqlx 性能提升 10%（从 10.04 μs 降至 9.06 μs），得益于：
- 泛型 SqlQuery<T> 的 EntityProvider 缓存优化
- DynamicResultReader 静态方法缓存（IDataRecord.GetInt32/GetString 等）
- 完全消除运行时反射查找

**AOT 兼容性：** ✅ 完全支持 Native AOT
- 通过 817 个单元测试（100% 通过率）
- 使用表达式树编译，无运行时反射
- 静态方法缓存，零动态代码生成
