...
public interface IAdvancedSqlTestRepository
{
    ...
    [SqlTemplate("SELECT city as City, COUNT(*) as order_count, SUM(amount) as total_amount FROM orders GROUP BY city")]
    Task<List<AdvancedSqlOrderSummary>> GetOrderSummaryByCityAsync();

    [SqlTemplate("SELECT city as City, 0 as order_count, SUM(amount) as total_amount FROM orders GROUP BY city HAVING SUM(amount) > @minAmount")]
    Task<List<AdvancedSqlOrderSummary>> GetHighValueCitiesAsync(int minAmount);

    [SqlTemplate("SELECT id, name, category, 0 as price, '' as price_category FROM products WHERE category IN ('Tools', 'Electronics')")]
    Task<List<AdvancedSqlProduct>> GetProductsInCategoriesAsync();

    [SqlTemplate("SELECT id, name, '' as category, 0 as price, '' as price_category FROM products WHERE name LIKE '%' || @pattern || '%'")]
    Task<List<AdvancedSqlProduct>> GetProductsContainingAsync(string pattern);

    [SqlTemplate("SELECT id, name, '' as category, price, '' as price_category FROM products WHERE price BETWEEN @minPrice AND @maxPrice")]
    Task<List<AdvancedSqlProduct>> GetProductsInPriceRangeAsync(int minPrice, int maxPrice);

    [SqlTemplate("SELECT id, name, '' as category, price, CASE WHEN price < 100 THEN 'Low' WHEN price < 200 THEN 'Medium' ELSE 'High' END as price_category FROM products")]
