...
## v2.0.1 - 2025年9月11日 - 🔧 重要修复版本

### 🐛 关键问题修复

#### 批处理实现修复
- **修复**: `batchxxxx` 实现错误，现在正确使用 `CreateBatchCommand` API
- **影响**: 批处理操作现在使用真正的 `DbBatch` API，而不是逐个执行
- **性能提升**: 在支持的数据库上可获得 **10-100倍** 性能提升

#### CRUD字段命名方言修复  
- **修复**: 所有 CRUD 操作不再固定使用 SQL Server 方言
- **新功能**: 智能数据库方言检测，根据连接类型自动选择：
...
#### 主构造函数参数映射修复
- **修复**: 主构造函数参数在 INSERT 操作中缺失的问题
- **改进**: 增强 `GetInsertableProperties` 方法，正确包含主构造函数参数

#### NameMapper 命名规则修复
- **修复**: `MapName` 方法恢复蛇形命名法转换功能
- **行为**: C# Pascal/camelCase 自动转换为 database snake_case
...
## v2.0.0 - 2025年9月11日 - 🚀 重大更新：现代 C# 支持
