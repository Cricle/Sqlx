...
public static class SqlTemplateEngineExtensions
{
    ...
    public static class MultiDatabasePlaceholderSupport
    {
        ...
        public static string ProcessLimitPlaceholder(string type, string options, SqlDefine dialect)
        {
            ...
            if (dialect.Equals(SqlDefine.SqlServer))
                ...
                return $"LIMIT {limit}";  // MySQL, PostgreSQL, SQLite
        }
    ...
    public static class DatabaseSpecificFeatures
    {
        ...
        public static string GetCurrentDateFunction(SqlDefine dialect)
        {
            ...
            {
                var d when d.Equals(SqlDefine.SqlServer) => "GETDATE()",
                var d when d.Equals(SqlDefine.MySql) => "NOW()",
                var d when d.Equals(SqlDefine.PostgreSql) => "NOW()",
                var d when d.Equals(SqlDefine.SQLite) => "datetime('now')",
                var d when d.Equals(SqlDefine.Oracle) => "SYSDATE",
                _ => "NOW()"
            };
        }
        ...
        public static string GetConcatSyntax(SqlDefine dialect, params string[] parts)
        {
            return dialect switch
            {
                var d when d.Equals(SqlDefine.SqlServer) => $"CONCAT({string.Join(", ", parts)})",
                var d when d.Equals(SqlDefine.MySql) => $"CONCAT({string.Join(", ", parts)})",
                var d when d.Equals(SqlDefine.PostgreSql) => string.Join(" || ", parts),
                var d when d.Equals(SqlDefine.SQLite) => string.Join(" || ", parts),
                var d when d.Equals(SqlDefine.Oracle) => string.Join(" || ", parts),
        ...
        public static string GetAutoIncrementSyntax(SqlDefine dialect)
        {
            return dialect switch
            {
                var d when d.Equals(SqlDefine.SqlServer) => "IDENTITY(1,1)",
                var d when d.Equals(SqlDefine.MySql) => "AUTO_INCREMENT",
                var d when d.Equals(SqlDefine.PostgreSql) => "SERIAL",
                var d when d.Equals(SqlDefine.SQLite) => "AUTOINCREMENT",
                var d when d.Equals(SqlDefine.Oracle) => "GENERATED BY DEFAULT AS IDENTITY",
                _ => "AUTO_INCREMENT"
            };
        }
    }
