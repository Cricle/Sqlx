...
namespace Sqlx.Tests.Core
{
    ...
    public class ComprehensiveCoverageTests
    {
        ...
        {
            // Act - Test that all types in Sqlx.Annotations namespace are accessible
            var annotationsAssembly = typeof(Sqlx.Annotations.SqlxAttribute).Assembly;
            var annotationTypes = annotationsAssembly.GetTypes()
                .Where(t => t.Namespace == "Sqlx.Annotations")
                .ToArray();

            // Assert - Check that we have the expected types
            Assert.IsTrue(annotationTypes.Length >= 8, "Should have at least 8 types in Sqlx.Annotations namespace");
            
            // Check for required attribute types
            Assert.IsTrue(annotationTypes.Any(t => t.Name == "SqlxAttribute"), "SqlxAttribute should exist");
            Assert.IsTrue(annotationTypes.Any(t => t.Name == "RawSqlAttribute"), "RawSqlAttribute should exist");
            Assert.IsTrue(annotationTypes.Any(t => t.Name == "ExpressionToSqlAttribute"), "ExpressionToSqlAttribute should exist");

            // Check for enum types and classes
            var classCount = annotationTypes.Count(t => t.IsClass && !t.IsAbstract);
            var staticClassCount = annotationTypes.Count(t => t.IsClass && t.IsAbstract && t.IsSealed);
            var enumCount = annotationTypes.Count(t => t.IsEnum);

            Assert.IsTrue(classCount > 0, "Should have classes");
            Assert.IsTrue(staticClassCount > 0, "Should have static classes");
            Assert.IsTrue(enumCount >= 2, "Should have at least 2 enums (SqlExecuteTypes, SqlDefineTypes)");

            // Check for specific types
            var sqlTemplateType = annotationTypes.FirstOrDefault(t => t.Name == "SqlTemplate");
            var expressionToSqlType = annotationTypes.FirstOrDefault(t => t.Name.StartsWith("ExpressionToSql"));
            
            Assert.IsNotNull(sqlTemplateType, "SqlTemplate should exist");
            Assert.IsNotNull(expressionToSqlType, "ExpressionToSql should exist");
        }
