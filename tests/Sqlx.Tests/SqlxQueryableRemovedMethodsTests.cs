...
public class SqlxQueryableRemovedMethodsTests
{
    ...
    {
        var queryable = SqlQuery<TestEntity>.For(SqlDefine.SQLite).Where(e => e.Id == 1);
        Assert.IsNotNull(queryable);
        Assert.IsInstanceOfType(queryable, typeof(IQueryable<TestEntity>));
    }
    ...
    {
        var queryable = SqlQuery<TestEntity>.For(SqlDefine.SQLite).OrderBy(e => e.Id);
        Assert.IsNotNull(queryable);
    }
    ...
    {
        var queryable = SqlQuery<TestEntity>.For(SqlDefine.SQLite).Select(e => new { e.Id, e.Name });
        Assert.IsNotNull(queryable);
    }
