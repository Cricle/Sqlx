...
|------|------|------------|---------|
| 单条查询 | **8.70 μs** | 10.35 μs (+19%) | 59.30 μs (+582%) |
| 内存分配 | **1.41 KB** | 2.66 KB (+89%) | 10.24 KB (+626%) |

**AOT 兼容性：** ✅ 完全支持 Native AOT，通过 1277 个单元测试
...
|--------|------|-------|--------|-------|-----------|-------------|
| **Sqlx** | **8.70 μs** | 0.16 μs | 0.15 μs | **1.00** | **1.41 KB** | **1.00** |
| Dapper.AOT | 10.35 μs | 0.09 μs | 0.08 μs | 1.19 | 2.66 KB | 1.89 |
| FreeSql | 59.30 μs | 0.54 μs | 0.51 μs | 6.82 | 10.24 KB | 7.28 |

**结论：** Sqlx 比 Dapper.AOT 快 19%，比 FreeSql 快 6.8 倍。

**性能提升：** 相比之前版本，Sqlx 性能进一步优化（8.70 μs），得益于：
...
- ✅ 通过 1277 个单元测试，Native AOT 就绪
- ✅ SQL 生成完全无反射（移除 GetEntityProvider 反射）
- ✅ 支持 JOIN 查询，无性能损失
- ✅ 使用 SqlDialect 方法复用分页逻辑
