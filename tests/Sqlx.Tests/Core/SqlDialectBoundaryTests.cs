...
namespace Sqlx.Tests.Core
{
    ...
    public class SqlDialectBoundaryTests
    {
        ...
        [TestMethod]
        public void SqlDialect_PerformanceUnderStress_WithinLimits()
        {
            // Arrange
            ...
            foreach (var (dialectName, dialect) in dialects)
            {
                ...
                try
                {
                    ...
                    {
                        // 测试各种操作
                        var column = dialect.WrapColumn($"Column{i}");
                        var str = dialect.WrapString($"String{i}");
                        var param = dialect.CreateParameter($"param{i}");
                        var dbType = dialect.DatabaseType;
                        
                        // 检查超时
                        if ((DateTime.Now - startTime).TotalSeconds > timeoutSeconds)
                        {
                            Console.WriteLine($"⚠️ {dialectName}: 性能测试超时，完成 {i} 次迭代");
                            break;
                        }
                    }
                    
                    var duration = DateTime.Now - startTime;
                    var operationsPerSecond = iterations / duration.TotalSeconds;
                    
                    Console.WriteLine($"✅ {dialectName}: 性能测试完成");
                    Console.WriteLine($"   耗时: {duration.TotalMilliseconds:F2}ms");
                    Console.WriteLine($"   操作/秒: {operationsPerSecond:F0}");
                    
                    // 性能断言 - 应该能够每秒处理至少1000次操作
                    Assert.IsTrue(operationsPerSecond > 1000, 
                        $"{dialectName} 性能不足: {operationsPerSecond:F0} ops/sec < 1000");
                }
                catch (Exception ex)
                {
                    var duration = DateTime.Now - startTime;
                    Console.WriteLine($"⚠️ {dialectName}: 性能测试异常 (耗时 {duration.TotalSeconds:F2}s): {ex.GetType().Name}");
                }
            }
        }
