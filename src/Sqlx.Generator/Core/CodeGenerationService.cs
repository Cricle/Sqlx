...
public class CodeGenerationService
{
    ...
    public void GenerateRepositoryMethod(RepositoryMethodContext context)
    {
        ...
        try
        {
            ...
            {
                GenerateSqlDebuggerMethod(sb, method, entityType, resolvedTableName, templateResult, context.ClassSymbol);
            }
        }
    ...
    /// <summary>
    /// Generates a SQL debugger method that returns SqlTemplate without executing the query.
    ...
    private void GenerateSqlDebuggerMethod(
        ...
        INamedTypeSymbol? entityType,
        string tableName,
        SqlTemplateResult templateResult,
        INamedTypeSymbol classSymbol)
    {
        var debugMethodName = $"Get{method.Name}Sql";
        var processedSql = templateResult.ProcessedSql;

        // Convert runtime placeholders to readable format for both display and return value
        var displaySql = ConvertRuntimePlaceholdersToComments(processedSql);

        // Generate XML documentation with full SQL
        sb.AppendLine("/// <summary>");
        sb.AppendLine($"/// Gets the SQL template for {method.Name} without executing.");
        sb.AppendLine("/// <para>SQL:</para>");
        foreach (var line in displaySql.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
        {
            sb.AppendLine($"/// <para>  {System.Security.SecurityElement.Escape(line.Trim())}</para>");
        }
        sb.AppendLine("/// </summary>");
        sb.AppendLine("/// <returns>SqlTemplate with SQL and parameters.</returns>");

        // Generate method signature - no parameters needed, SQL is compile-time constant
        sb.AppendLine($"public global::Sqlx.SqlTemplate {debugMethodName}()");
        sb.AppendLine("{");
        sb.PushIndent();

        // Return SqlTemplate with the display SQL (runtime placeholders converted to comments)
        var escapedSql = displaySql.Replace("\"", "\"\"");
        sb.AppendLine($"return new global::Sqlx.SqlTemplate(@\"{escapedSql}\", new global::System.Collections.Generic.Dictionary<string, object?>());");
    ...
    /// <summary>
    /// Converts runtime placeholders to readable comments for documentation and display.
    /// </summary>
    private static string ConvertRuntimePlaceholdersToComments(string sql)
    {
        // Convert {{RUNTIME_WHERE_NATIVE_EXPR_xxx}} to /* WHERE: @xxx */
        sql = Regex.Replace(sql, @"\{\{RUNTIME_WHERE_NATIVE_EXPR_(\w+)\}\}", "/* WHERE: @$1 */");
        sql = Regex.Replace(sql, @"\{\{RUNTIME_WHERE_EXPR_(\w+)\}\}", "/* WHERE: @$1 */");
        sql = Regex.Replace(sql, @"\{\{RUNTIME_SET_EXPR_(\w+)\}\}", "/* SET: @$1 */");
        
        // Convert {RUNTIME_XXX_yyy} to /* XXX: @yyy */
        sql = Regex.Replace(sql, @"\{RUNTIME_(\w+)_(\w+)\}", "/* $1: @$2 */");
        
        // Convert remaining {{...}} placeholders
