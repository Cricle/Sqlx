// -----------------------------------------------------------------------
// <copyright file="MySQLFeaturePropertyTests.cs" company="Cricle">
...
/// <summary>
/// Property-based tests for MySQL-specific features.
/// **Feature: enhanced-predefined-interfaces-e2e-testing**
/// **Validates: Requirements 4.3, 4.8, 4.9**
/// </summary>
public class MySQLFeaturePropertyTests : IDisposable
{
    private readonly IDbConnection? _connection;
    private readonly E2EUserCommandRepository_MySQL? _commandRepo;
    private readonly E2EUserQueryRepository_MySQL? _queryRepo;
    private readonly E2EUserBatchRepository_MySQL? _batchRepo;
    private readonly bool _isAvailable;
    private const string TestClassName = nameof(MySQLFeaturePropertyTests);

    public MySQLFeaturePropertyTests()
    {
        _connection = DatabaseConnectionHelper.GetMySQLConnection(TestClassName);
        ...
        if (_isAvailable)
        {
            ...
            _commandRepo = new E2EUserCommandRepository_MySQL(_connection);
            _queryRepo = new E2EUserQueryRepository_MySQL(_connection);
            _batchRepo = new E2EUserBatchRepository_MySQL(_connection);
        }
    }

    /// <summary>
    /// **Property 18: Upsert Idempotence (MySQL-specific)**
    /// *For any* entity, upserting it multiple times should produce the same result
    /// as upserting it once (idempotent operation using ON DUPLICATE KEY UPDATE).
    ...
    /// <summary>
    /// **Property 30: Boolean Preservation (MySQL-specific)**
    /// *For any* entity with boolean fields, inserting and retrieving should preserve
    /// the exact boolean values (MySQL stores as 0/1).
    ...
    public Property Boolean_ShouldBePreservedInRoundTrip(E2EUserData userData)
    {
        ...
        // Assert - Boolean values should be preserved exactly (0/1 in MySQL)
    ...
    /// <summary>
    /// **Property 30b: Boolean Preservation in Batch Operations (MySQL-specific)**
    /// *For any* list of entities with boolean fields, batch inserting and retrieving
    /// should preserve all boolean values correctly (MySQL 0/1).
    /// **Validates: Requirements 4.8**
    /// </summary>
    [Property(MaxTest = 10, Arbitrary = new[] { typeof(E2EUserArbitraries) })]
    ...
    /// <summary>
    /// **Property 32: Random Query Validity (MySQL-specific)**
    /// *For any* non-empty table, GetRandomAsync should always return valid entities
    /// that exist in the table (using RAND() function).
    ...
    public Property GetRandom_ShouldReturnValidEntities(PositiveInt count, NonEmptyArray<E2EUserData> userDataArray)
    {
        ...
        // Act - Get random users using RAND()
    ...
    /// <summary>
    /// **Property 32b: Random Query Uniqueness (MySQL-specific)**
    /// *For any* table with sufficient records, GetRandomAsync should tend to return
    /// different results on multiple calls (statistical property using RAND()).
    /// **Validates: Requirements 4.9**
    /// </summary>
    ...
    public Property GetRandom_ShouldTendToReturnDifferentResults(NonEmptyArray<E2EUserData> userDataArray)
    {
        ...
        // Act - Get random users multiple times using RAND()
    ...
    /// <summary>
    /// **Property 18b: Batch Upsert Correctness (MySQL-specific)**
    /// *For any* list of entities (mix of existing and new), batch upsert should
    /// correctly update existing records and insert new ones using ON DUPLICATE KEY UPDATE.
    /// **Validates: Requirements 4.3**
    /// </summary>
    [Property(MaxTest = 10, Arbitrary = new[] { typeof(E2EUserArbitraries) })]
    public Property BatchUpsert_ShouldCorrectlyHandleMixedOperations(NonEmptyArray<E2EUserData> existingData, NonEmptyArray<E2EUserData> newData)
    {
        ...
        // Act - Batch upsert using ON DUPLICATE KEY UPDATE
