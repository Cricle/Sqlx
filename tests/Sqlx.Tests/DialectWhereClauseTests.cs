...
public class DialectWhereClauseTests
{
    ...
    [TestMethod]
    public void AllDialects_DateProperty_ConvertedToSnakeCase()
    {
        var date = new DateTime(2024, 1, 1);
        Expression<Func<Product, bool>> predicate = p => p.CreatedDate > date;
        ...
        Assert.IsTrue(mysqlSql.Contains("created_date"));
        Assert.IsTrue(pgsqlSql.Contains("created_date"));
        Assert.IsTrue(sqlserverSql.Contains("created_date"));
        Assert.IsTrue(sqliteSql.Contains("created_date"));
    }
    ...
    {
        // Use constant value directly (not captured variable)
