...
namespace TestNamespace
{
    ...
    public void CSharpGenerator_WithLargeInterface_PerformsWell()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            methodsBuilder.AppendLine($"        Entity{i}? GetEntity{i}ById(int id);");
            methodsBuilder.AppendLine($"        IList<Entity{i}> GetAllEntity{i}();");
            methodsBuilder.AppendLine($"        int CreateEntity{i}(Entity{i} entity);");
            methodsBuilder.AppendLine($"        int UpdateEntity{i}(Entity{i} entity);");
            methodsBuilder.AppendLine($"        int DeleteEntity{i}(int id);");
        }

        var entitiesBuilder = new System.Text.StringBuilder();
        for (int i = 0; i < 50; i++)
        {
